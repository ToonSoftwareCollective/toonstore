...
Item {
	...
	QtObject {
		...
		property ToonstoreThreeStateButton prvButUp: butUp
		property ToonstoreThreeStateButton prvButDown: butDown
		...
		function updateButtons() {
			...
			//had to do manual setting of visible property of buttons here because there is a problem with icon in ToonstoreThreeStateButton when directly bound
		...
		function updateScrollbar() {
			...
			scrollbar.height = Math.floor(trafficMarkup.height / pageCount);
			...
			topMargin = (topMargin + scrollbar.height) >= trafficMarkup.height ? trafficMarkup.height - scrollbar.height - 1 : topMargin;
	...
	Rectangle {
		...
		Column {
			id: repeaterColumn
			width: trafficMarkup.width
			height: trafficMarkup.height
			anchors {
				top: trafficMarkup.top
				topMargin: 6
				left: trafficMarkup.left
				leftMargin: 6
			}
			spacing: 5
	...
	ToonstoreThreeStateButton {
		id: butDown
		width: 38
		height: trafficMarkup.height / 2
		...
		anchors {
			bottom: trafficMarkup.bottom
			left: trafficMarkup.right
